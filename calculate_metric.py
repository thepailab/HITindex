...
def running_genmodel(exon_outname):
	...
	D = df_exon['nRIGHT'].values
	N = (df_exon['nLEFT'] + df_exon['nRIGHT']).values
...
def writeMetrics(HITdict, param, outname):
    ...
    outexon.write('exon\tgene\tstrand\tnTXPT\tnFE\tnINTERNAL\tnLE\tnSINGLE\tnLEFT\tnRIGHT\tHITindex\tboot_pval\tCI75_low\tCI75_high\tCI90_low\tCI90_high\tCI95_low\tCI95_high\tdist_to_TSS\tdist_to_TES\tedge\n')
    for gene in HITdict:
...
def calculatePSI(HITidentify, edge, outname):
	...
	for genehere in genelist:
		...
		### CALCULATE AFE
		RLlist = afehere['nRIGHT'] - afehere['nLEFT']
		if len(afehere) > 0 and sum(RLlist) > 0:
			...
			afehereparse = afehere[['gene', 'exon', 'strand', 'nFE', 'nLEFT', 'nRIGHT', 'HITindex', 'sumR-L', 'AFEPSI']]
			afepsidata.append(afehereparse)
		### CALCULATE ALE
		LRlist = alehere['nLEFT'] - alehere['nRIGHT']
		if len(alehere) > 0 and sum(LRlist) > 0:
			...
			# write afe psi values
			alehereparse = alehere[['gene', 'exon', 'strand', 'nFE', 'nLEFT', 'nRIGHT', 'HITindex', 'sumL-R', 'ALEPSI']]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog="HITindex", add_help = True)
    ...
    parser.add_argument('--identifyTerminal', action = 'store_true', help='Identify terminal, hybrid, and internal exons', required = False)
    parser.add_argument('--calculatePSI', action = 'store_true', help='Calculate PSI values', required = False)
    parser.add_argument('--outname', type=str, help='name of file(s) for final metric.', required=True)
    ### read information
    group_reads = parser.add_argument_group('read information')
    group_reads.add_argument('--bam', type = str, help = 'original bam from which to extract junction reads. required if --junctionReads', required=False, default="NotSpec")
    group_reads.add_argument('--juncbam', type = str, help = 'junction read bam. required if --junctionReads or --HITindex', required = False, default = "NotSpec")
    group_reads.add_argument('--readtype', type=str, help = 'type of read (default = paired)', default='paired', required=False, choices=['single','paired'])
    group_reads.add_argument('--readstrand', type=str, help = 'directionality of RNA-seq data (default = fr-firststrand)', default='fr-firststrand', required=False, choices=['fr-unstrand','fr-firststrand','fr-secondstrand'])
    ### exon information
    group_exons = parser.add_argument_group('exon information')
    group_exons.add_argument('--bed', type = str, help = 'bed file with merged/annotated exons. required if --HITindex', required = False, default = "NotSpec")
    ## HITindex parameters
    group_param = parser.add_argument_group('HIT', 'parameters for running HIT index')
    group_param.add_argument('--overlap', type = int, help = 'overlap of split read with exon region (nt). default = 10', required = False, default = 10)
    group_param.add_argument('--readnum', type = int, help = 'minimum number of reads for confidence in HITindex (sum of R + L). default = 2.', required = False, default = 2)
    group_param.add_argument('--bootstrap', type = int, help = 'bootstrapping iterations to get p-value for metric confidence (within 0.1). default = 1000', required = False, default = 1000)
    ### thresholds for calling terminal exons
    group_term = parser.add_argument_group('identify', 'information for identifying exon types') 
    group_term.add_argument('--metrics', type = str, help = 'HITindex output file, required if --HITindex is not specified.', required = False, default = "NotSpec")
    group_term.add_argument('--parameters', type = str, help = 'file specifying HITindex and generative model thresholds for identifying exons (default: HIT_identity_parameters.txt', required=False, default="HIT_identity_parameters.txt")
    ### PSI values
    group_psi = parser.add_argument_group('psi', 'parameters for calling PSI values')
    group_psi.add_argument('--metricsID', type = str, help = 'HITindex identification output file, required if --identifyTerminal is not specified.', required = False, default = "NotSpec")
    group_psi.add_argument('--edge', type = bool, help = 'exclude exons flagged as being ameffected by the edge effect from PSI calculations', required = False, default = False)
    # confidence information
    args = parser.parse_args()

    if not args.HITindex and not args.identifyTerminal and not args.calculatePSI:
    	sys.exit("ERROR! Need to include at least one function: --HITindex, --identifyTerminal, and/or --calculatePSI")

    #pdb.set_trace()
	# dill.dump_session('globalsave.pkl')

	if args.junctionReads:
		if args.bam == "NotSpec" or args.juncbam == "NotSpec":
    ...
    if args.HITindex:
    	if args.juncbam == "NotSpec" or args.bed == "NotSpec":
    ...
    if args.calculatePSI:
    	if not args.identifyTerminal:
    		if args.metricsID == "NotSpec":
