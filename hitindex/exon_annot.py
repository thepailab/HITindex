...
if __name__ == '__main__':
    ...
    # index gtf
    parser.add_argument('--gtf', type = str, help = 'gtf to be indexed. required if --index', required=True)
    parser.add_argument('--reverse', action="store_true", help = 'use if exons are sorted by transcriptional direction rather than reference coordinate', required = False)
    parser.add_argument('--outfile_gtf', type = str, help = 'name for output bed with merged/annotated exons. required if --index', required = True)
    parser.add_argument('--ss3buffer', type = int, help = 'intronic buffer region included upstream of 3ss of exon for counting reads. default = 0nt. suggested = 50nt.', required = False, default = 0)
    parser.add_argument('--ss5buffer', type = int, help = 'intronic buffer region included downstream of 5ss of exon for counting reads. default = 0nt. suggested = 20nt.', required = False, default = 0)
